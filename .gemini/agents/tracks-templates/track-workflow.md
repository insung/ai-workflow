...
|---------------|-----------|--------|------|
| `plan.md` | 필수 | [plan-template.md](.agents/tracks-templates/plan-template.md) | 체크박스 형태의 작업 목록 |
| `audit.md` | 필수 | [audit-template.md](.agents/tracks-templates/audit-template.md) | 사용자 지시의 시간순 기록 |
| `strategy.md` | 선택사항 | [strategy-template.md](.agents/tracks-templates/strategy-template.md) | 기술적 결정 및 근거 |
| `notes/` | 선택사항 | - | 공통/방대한 구현 노트 |
| `todos/` | 선택사항 | - | 세부 체크리스트 |

- `plan.md` 생성 시 체크리스트 위에 **계획 워크플로우**를 ASCII 화살표로 작성하라.
  - 트랙 완료까지의 전체 흐름을 스케치 수준으로 표현한다.
  - 형식: `단계A → 단계B → 단계C`

## Rule 3: 월별 Status 갱신
- 새 트랙 생성 시: `tracks/YYYY-MM-status.md`에 항목 추가 (상태: `미착수`)
- 트랙 재개 시 → 상태가 `미착수`이면 `진행 중`으로 변경
- 월별 status 파일이 없으면 [status-template.md](.agents/tracks-templates/status-template.md)로 생성하라.
...
- 사용자 승인 시:
  ...
  2. plan.md에 **실행 워크플로우**를 ASCII 화살표로 추가하라.
     - 계획 워크플로우 아래에 배치한다.
     - 사용자 요구사항 변화 등을 반영한 실제 진행 흐름을 기록한다.
  3. "트랙 완료" 출력 템플릿 사용
  4. 월별 status를 `✅ 완료`로 갱신
- 출력 템플릿: [output-template.md](.agents/tracks-templates/output-template.md) 참조

## Rule 8: 구 구조 마이그레이션
- 트랙 검색 시 `tracks/` 직하에 `YYYY-MM-DD_HHMM_*` 패턴의 디렉토리가 존재하면 구 구조로 판단하라.
- 구 구조 감지 시 해당 트랙을 `tracks/YYYY-MM/DD_HHMM_{트랙명}/`으로 이동하라.
- 이동 후 `tracks/YYYY-MM-status.md`의 트랙 경로를 신규 경로로 갱신하라.
- 마이그레이션은 해당 트랙에 접근하는 시점에 수행한다 (일괄 이동 아님).
